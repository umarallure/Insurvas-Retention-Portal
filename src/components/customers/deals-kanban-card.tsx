...
export function DealsKanbanCard({ deal, clickable = true }: { deal: DealsKanbanRow; clickable?: boolean }) {
  ...
  const content = (
    <div className="min-w-0 flex-1 flex flex-col gap-3">
      <div
        className={"text-xl font-bold text-foreground truncate " + (clickable ? "group-hover:text-primary transition-colors" : "")}
      ...
      <div className="mt-auto space-y-2 text-base">
        <div className="flex items-center justify-between gap-2">
          ...
          <span className="font-bold text-foreground tabular-nums truncate text-right" title={deal.phone_number ?? undefined}>{deal.phone_number ?? "—"}</span>
        </div>
        ...
        <div className="flex items-start justify-between gap-2">
          ...
          <span className="font-semibold text-foreground text-right truncate text-sm leading-snug" title={deal.ghl_stage ?? undefined}>{deal.ghl_stage ?? "—"}</span>
        </div>
      </div>
    </div>
  );
  ...
  return <div className="text-left rounded-lg border bg-card p-4 shadow-sm w-full flex flex-col">{content}</div>;
}
